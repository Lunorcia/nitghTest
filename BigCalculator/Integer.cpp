...
Integer Integer::operator =(string a)//a是數字(單純用來修改this的數值
{
	if (this->name != "") {/*如果this有名字*/
		int pos = searchIntName(this->name);/*查找nameIntList內是否有this的名字，同步修改static的值*/
		Integer temp(a);//先把數字做處理(去前置0&小數點)
		this->numInt = temp.numInt;/*指派temp的數字(a)給this*/
		numIntList[pos] = this->numInt;/*如果this的名字存在，更新數值*/
	}
	return *this;
}
