...
string number::operator /(number K) {
    ...
    {
        bool allZeroTF = 1;
        for (int i = bit + 1; i < b.num.length(); i++)////浪代b计翴琌0
        {
            if (b.num[i] != '0')
            {
                allZeroTF = 0;
    ...
    //璸衡筁祘
    while (intIsBiggerOrEqual(a.num, b.num))//Α埃猭a玡计㎝b暗搭猭 
    {
        int n = 0;//Ωn计
        string partA="", remainA="";
        while (intIsBiggerOrEqual(b.num, partA) && b.num != partA)//bゑpartA碞计(&&逼埃b=a)讽partAì镑砆搭挡癹伴
        {
            n++;
        ...
        while (intIsBiggerOrEqual(partA, b.num))//搭猭衡坝
        {
            number tempA(partA);
        ...
        if (partA == "0")
        {
            for (int i = 0; i < n - 1; i++)//n坝干0
                ans += '0';
        }
        else
        {
            for (int i = 1; i < n - 1; i++)//n坝干0
                ans += '0';
        }
        ans += to_string(quotient);
        a.num = partA + remainA;
        if (a.num[0] == '0' && a.num.length() > 1)//partA砆俱埃杠秨繷0
            a.num = a.num.substr(1, a.num.length() - 1);
    }
    while (ans[0] == '0' && ans.length() > 1)//ans秨繷0(材Ω癹伴坝干0玻ネ)
