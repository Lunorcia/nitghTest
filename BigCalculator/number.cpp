...
string number::operator ^(number K) {
	...
	else if (isSquareRoot(b) == false) { /*如果b不為0.5的整數倍*/
		b.num.erase(b.num.begin() + bit, b.num.end());
		bit = b.num.find(".");
		b_dp_change = true;
	}

	if (ait == a.num.npos && bit == b.num.npos) { /*底數和指數皆為整數，可直接運算*/
	...
	else if (ait != a.num.npos && bit == b.num.npos) { /*底數為小數，指數為整數，結果應為小數*/
		/*for (int i = a.num.length() - 1; i > ait + 1; i--) {
			if (a.num[i] == '0')
		}*/
		unsigned long long int aDecDigits = a.num.length() - 1 - ait;/*紀錄小數點後的位數*/
		...
		if (ansDecDigits > 100)
			...
			for (int i = 0; i < 100 - ansDecDigits; i++)
				ans.num += "0";
		if (b_sign == false)
			ans.num = number("1") / ans;
		if (a_sign == false && ans.num != "0")
	...
	else if (isSquareRoot(b) == true) { /*如果b為0.5整數倍的小數*/
		if (a.num == "0") {
			number result("0");
		...
		unsigned long long int result_dp;  /*紀錄計算結果(result)的小數點位置*/
		number _a(a.num);
		...
		if (b.num != "0") {
			...
			if (ansDecDigits > 100) /*小數點後若超過100位，則刪除超過的部分*/
				ans.num.erase(ans.num.begin() + ans.num.find(".") + 101, ans.num.end());
			unsigned long long int ansit = ans.num.find(".");
