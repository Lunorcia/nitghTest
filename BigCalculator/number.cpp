...
string number::operator ^(number K) {/*負數不能開根號，但還是有算，目前註解掉了*/
	number a(num), b(K.num);
	unsigned long long int ait = a.num.find(".");
	unsigned long long int bit = b.num.find(".");
	...
	if (ait == a.num.npos && bit == b.num.npos) { /*底數a和指數b皆為整數，可直接運算*/
		...
		if (ans.num.find(".") == 0)/*若小數點在首位，補0*/
			ans.num.insert(0, 1, '0');
		if (b_sign == false)
			ans.num = number("1") / ans;
		if (ans.num.length() - 1 - ans.num.find(".") > 100) /*小數數量大於100，刪除多餘數字*/
			ans.num.erase(ans.num.begin() + ans.num.find(".") + 101, ans.num.end());
		if (ans.num.length() - 1 - ans.num.find(".") < 100)
			ans.num.insert(ans.num.end(), 100 - (ans.num.length() - 1 - ans.num.find(".")), '0');
		if (a_sign == false) {
			bool allZero = false; /*allZero：紀錄是否小數點後全為0*/
			if (ans.num[0] == '0') {
				allZero = true;
			...
			if (allZero == false) /*如果allZero為false，加上負號*/
				ans.num.insert(0, 1, '-');
		}
		return ans.num;
	}
		...
		if (ait == a.num.npos) { /*整數*/
			...
			resultit = tmp.size();
			for (int i = 0; i < 126; i++) { /*開根號可能有小數，先預留200位小數*/
				tmp.push_back(0);
			}
		}
		else { /*小數*/
			...
			for (int i = 0; i < 252 - decimal_len; i++) { /*如果decimal_len為奇數，則補0到小數點後第400位*/
				_a.num.push_back('0');
			}
			for (int i = ait + 1; i < _a.num.length(); i = i + 2) { /*小數位倆倆一組，分割後會儲存成200位*/
				string n;
				n.push_back(_a.num[i]);
				n.push_back(_a.num[i + 1]);
				tmp.push_back(stoi(n));
		...
		number ans("1");
		number count("0");
		while (count - b != "0") { /*如果b的整數部分大於0，則做指數運算*/
			ans.num = ans * a;
			count.num = count + number("1");
			ansDecDigits += aDecDigits;/*每乘一次，結果的小數點後位數增加*/
		}
		ans.num = result * ans; /*合併*/
		ansDecDigits += result.num.length() - resultit; /*紀錄結果的小數長度*/
		if (ansDecDigits > ans.num.length())/*還原前方刪除的0*/
			ans.num.insert(0, ansDecDigits - ans.num.length() + 1, '0');
		ans.num.insert(ans.num.end() - ansDecDigits, 1, '.'); /*還原小數點位置*/
		if (ans.num.find(".") == 0) /*如果ans的首位為小數點，補0*/
			ans.num.insert(0, 1, '0');
		if (b_sign == false)
			ans.num = number("1") / ans;
		if (ans.num.length() - 1 - ans.num.find(".") > 100) /*小數數量大於100，刪除多餘數字*/
