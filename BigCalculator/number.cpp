...
string number::operator ^(number K) {
	...
	if (ait == a.num.npos && bit == b.num.npos) { /*底數a和指數b皆為整數，可直接運算*/
		...
		if (ansDecDigits > 100) /*若小數數量大於100，刪除多餘數字*/
			ans.num.erase(ans.num.begin() + ans.num.find(".") + 101, ans.num.end());
		else if (ansDecDigits < 100) /*若小數數量小於100，補0*/
			ans.num.insert(ans.num.end(), 100 - ansDecDigits, '0');
		if (b_sign == false) {
			ans.num = number("1") / ans;
			if (ans.num.length() - 1 - ans.num.find(".") > 100) /*若除法後，小數數量大於100，刪除多餘數字*/
				ans.num.erase(ans.num.begin() + ans.num.find(".") + 101, ans.num.end());
		}
		if (a_sign == false) {
		...
		if (ait == a.num.npos) { /*整數*/
			...
			resultit = tmp.size();
			for (int i = 0; i < 101; i++) { /*開根號可能有小數，先預留100位小數*/
				tmp.push_back(0);
			}
		}
		else { /*小數*/
			...
			for (int i = 0; i < 202 - decimal_len; i++) { /*如果decimal_len為奇數，則補0到小數點後第200位*/
				_a.num.push_back('0');
			}
