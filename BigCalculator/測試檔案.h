...
string number::operator /(number K) {
    ...
    for (int i = 0; i < len+100; i++)//氮皚耴箂
        numAns[i] = 0;
    ...
    for (MSD; MSD < a.num.length-lenb; MSD++)
    {
        for (int i = MSD,int iB=0; i < b.num.length(); i++,iB)//眖程蔼计オ计秨﹍絋粄
        {
            if (numA[i] > numB[iB])               //狦程蔼计>埃计程蔼计玥﹚搭
            {
                ...
                i = MSD;                        //搭Ч程オ娩 穝浪琩//////////////////////////////////癹伴i穦癹伴琌眖i=MSD+1秨﹍耞ぃ琌程オ娩翅
                iB = 0;
            }
        }
    }
    ...
    if (checkDecA == true)//挡狦琌计计翴干
    {
        for (int i = 0; i < countDec; i++)//计
            ans += to_string(numAns[i]);
        ans += ".";
        for (int i = countDec; i < len; i++)//俱计
            ans += to_string(numAns[i]);
