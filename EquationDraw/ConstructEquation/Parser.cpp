...
void Parser::setEquationPart(string input) { //儲存等式
	...
	for (int i = 0; i < input.size(); i++) {
		...
		else if (((input[i + 1] < '0' || input[i + 1] > '9') && input[i + 1] != '.') && isDigit(s) == true) {//數字
			formula.push_back(s);
			s.clear();
		}
		...
		else if (input[i + 1] == '=' || input[i + 1] == '(' || input[i + 1] == ')' || input[i + 1] == '+' || input[i + 1] == '-' || input[i + 1] == '*' || input[i + 1] == '/' || input[i + 1] == '^') { //變數
			formula.push_back(s);
			s.clear();
		}
		if (i == input.size() - 1) {
	...
	if (valid == false) //輸入數字檢測不符規定
		...
		if (checkDefinedVariable(cv) == false) { //存在未定義變數
			input = "error";
			cout << "存在未定義變數" << endl;
		}
	}
